...
export default function BasicFee({ pharmData, isDesktop, excluded = {}, onSetExcluded }) {
  ...
  if (!periods.length) {
    return (
      <div style={{ background: "#fff", border: `1px solid ${C.border}`, borderRadius: 16, padding: "22px 18px", textAlign: "center", color: C.sub, fontSize: 13 }}>
  ...
  return (
    <>
      {/* 説明 */}
      <div style={{ background: "linear-gradient(180deg, #EBF0F5 0%, #fff 75%)", border: `1px solid ${C.border}`, borderRadius: 16, padding: "14px 15px", marginBottom: 14, boxShadow: "0 3px 12px rgba(18,48,46,.05)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7, fontFamily: FONT_SERIF, fontSize: 15, fontWeight: 600, color: C.ink, marginBottom: 6 }}>
          ...
          {periods.map((p) => (
            <button
              ...
              style={{
                ...
                border: p === P ? "none" : `1px solid ${C.border}`,
                background: p === P ? `linear-gradient(135deg, ${C.teal}, ${C.tealBright})` : "#fff",
                color: p === P ? "#fff" : C.sub,
                fontWeight: 800, fontSize: 12.5, cursor: "pointer",
                boxShadow: p === P ? "0 3px 10px rgba(14,124,123,.26)" : "0 1px 2px rgba(0,0,0,.04)",
              }}
            >
              {`'${yy(p)}/5〜'${yy(p + 1)}/4`}
            </button>
      ...
      {/* 出典・正式ルール */}
      <div style={{ background: "#fff", border: `1px solid ${C.border}`, borderRadius: 14, padding: "13px 15px", marginTop: 16, fontSize: 11, color: C.sub, lineHeight: 1.8 }}>
...
function PharmacyCard({ pharm, P, excluded, onSetExcluded }) {
  ...
  return (
    <div style={{ background: "#fff", border: `1px solid ${C.border}`, borderRadius: 18, padding: "16px 16px 12px", boxShadow: "0 3px 14px rgba(18,48,46,.05)" }}>
      {/* ヘッダ */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: 12 }}>
        <div style={{ fontFamily: FONT_SERIF, fontSize: 16, fontWeight: 600, color: C.ink }}>{pharm.name}</div>
      ...
      {/* 判定ロジック */}
      <div style={{ background: isBasic2 ? "#FBEBEB" : "#F2F6F1", borderRadius: 10, padding: "9px 11px", fontSize: 11.5, color: C.ink, lineHeight: 1.6, marginBottom: 12 }}>
      ...
      <ResponsiveContainer width="100%" height={170}>
        <ComposedChart data={chartData} margin={{ top: 10, right: 6, left: -6, bottom: 0 }}>
          ...
          <Tooltip formatter={(v) => [v == null ? "未登録" : v.toLocaleString("ja-JP") + "枚", "受付(除外後)"]} contentStyle={{ borderRadius: 10, border: "none", boxShadow: "0 4px 14px rgba(0,0,0,.1)", fontSize: 12 }} />
          ...
          <Bar dataKey="adj" radius={[4, 4, 0, 0]} maxBarSize={20}>
            {chartData.map((r, i) => (
              <Cell key={i} fill={r.adj == null ? "#eef2f5" : r.adj > RECEIPT_LIMIT ? C.down : C.tealBright} />
            ))}
          </Bar>
        </ComposedChart>
      </ResponsiveContainer>
              ...
              {rows.map((r) => (
                <div key={r.period} style={{ display: "flex", alignItems: "center", gap: 6, opacity: r.total == null ? 0.5 : 1 }}>
                  ...
                  <input
                    ...
                    inputMode="numeric"
                    placeholder="除外0"
                    style={{ flex: 1, minWidth: 0, height: 34, border: `1px solid ${C.line}`, borderRadius: 8, padding: "0 8px", fontSize: 13, textAlign: "right", boxSizing: "border-box" }}
                  />
                </div>
              ))}
            </div>
...
function MetricTile({ label, value, unit, sub, over, limitText }) {
  ...
  return (
    <div style={{ background: over ? "#FBEBEB" : "#F2F6F1", borderRadius: 12, padding: "10px 11px", border: `1px solid ${col}22` }}>
